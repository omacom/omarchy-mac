...
Omarchy recommends three ways of running local LLM models: LM Studio, Ollama, and llmman. LM Studio provides a GUI interface for finding open-weight models, installing them, and running them. It's a great way to get going easily. Ollama offers a CLI for doing so similarly. But if you're new to local models, I'd start with LM Studio. You can install any of them under _Install > AI_ in the Omarchy Menu.

[llmman](https://github.com/llmmanorg/llmman) is the one for pointing the coding agents above at a local model: `llmman launch claude --model qwen3.8` fetches the tested upstream `llama.cpp` build for your GPU, loads the model, and starts Claude Code against it (the same works for `opencode`, `codex`, `agy`, and others). Models are plain OCI artifacts, pulled straight from Docker Hub or Hugging Face, and `llmman serve` answers the Ollama, OpenAI, and Anthropic APIs on one endpoint, so anything that already talks to Ollama works unchanged. Swap `--model` for `--provider` to run the same agent against a hosted model instead.
