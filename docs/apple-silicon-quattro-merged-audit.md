...
- 72 PRs: 26 present, 31 superseded, 5 missing, 10 not Apple. Two bundles also hide missing parts: #435's fresh-install zram default and #389's setup-failure message.
...
- Bugs confirmed on the converged target: Brave plays no DRM video on Macs (#474), Voxtype setup prints a Vulkan error on every Mac (#483), first-run retries forever while omacom's `omarchy-settings` omits the keyboard ALS unit (#432), and fresh Mac images get no zram swap (#435, U5).
- The legacy cohort differs from what 44 and 45 assume. Since #155 the `quattro` guided installer encrypts by default, with busybox `encrypt`, `cryptdevice=` and `/boot` on the ESP; rc4 installed a fully trusted fork keyring. See the legacy findings and M2.
...
| [#387](https://github.com/omacom/omarchy-mac/pull/387) | Supply a zram default when nothing configures it, and never block the migration queue | Reworked in `4d767bc68` (`migrations/1789154627.sh` L13-L35). Differences for 14: `qu` checks three locations (#387 checks every generator directory), treats a `/dev/null` symlink as unconfigured, and starts `systemd-zram-setup@zram0.service` instead of `dev-zram0.swap`, so swap waits for a reboot. |
...
2. **Fork signing trust.** rc4 (#435 `fea4693e8`, #444; migrations `1789316115`, `1789407944`) installs `omarchy-mac-keyring` and trusts `FBD6874D423C418DDB6D143EECE19CDDE306DBD2` at full trust, while `[omarchy-aarch64]` stays `SigLevel = Optional TrustAll`. 44 must remove the package and the key, as 43 removes mx's `C81AC3E2…` (gap audit R2), and handle rc5 if it ships (triage #436).
