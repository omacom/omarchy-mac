...
| Early M3 desktop and launch kernel selection | [Kernel qualification and delivery proposal in the plan](upstream-integration-plan.md#kernel-graphics-and-hardware-scope), existing boot/installer efforts and the main unified-installer beta scope | Target Aurora from [omacom/linux](https://github.com/omacom/linux), recording the hardware qualification and actual package delivery needed for launch. Use Asahi only if timing requires a fallback, and plan the subsequent transition to Aurora. Add separate early-hardware delivery/validation tasks when the proposed package paths are agreed. The [M3 MLX/ANE card](https://app.basecamp.com/5994298/buckets/48646031/card_tables/cards/10320790748) is a different capability and must not become a dependency for a software-rendered M3 desktop |
