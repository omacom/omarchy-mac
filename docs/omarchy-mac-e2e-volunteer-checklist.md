...
Use the exact command assigned by the test coordinator. For a recorded E2E run on the current default branch (captures the fresh Asahi baseline, then starts guided setup):

```bash
curl -fsSL https://raw.githubusercontent.com/omacom/omarchy-mac/quattro/bin/omarchy-mac-e2e-collect | bash
```

Setup alone, without evidence collection:

```bash
curl -fsSL https://raw.githubusercontent.com/omacom/omarchy-mac/quattro/bin/omarchy-mac-setup | bash
```

See [`e2e-collector.md`](e2e-collector.md) for the collector's volunteer flow.
