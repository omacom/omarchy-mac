...
- No environment variable changes what the guard does: as root it ignores exported shell functions and restarts in an empty environment before reading anything. To turn it off on purpose, mask it: `ln -s /dev/null /etc/pacman.d/hooks/00-omarchy-platform-guard.hook`. A VM running an Apple image needs this when its device tree is not a Mac's, or updates of its Apple packages are refused. `tools/acceptance/mac-image` needs no mask: it gives the guest's device tree an Apple root compatible, so the guard sees `apple-silicon` there.
...
2. Platform packages come in a later transaction: the Apple list of `omarchy-pkg-defaults apple-silicon`, the Apple kernel and the boot chain included, so none of them may be in a first pacstrap. An Apple image builder must install the settings package first, write the image manifest, then install the Apple set. The Mac image builder (`image-builder/bin/build-mac-image` in omacom/omarchy-mac-installer) does: the base system with the settings package, then the manifest, the runtime with the base and aarch64 lists, and the Apple set last. It takes the runtime and settings packages from the same source commit at the same version, but the settings recipe decides whether the guard ships: a candidate set built from a source with this step needs a settings recipe from omacom/omarchy-pkgs #636 or later, or its image's first boot holds every hardware step behind this one. The mx-mac era builders do not follow this order: they pacstrap the Aurora kernel, m1n1 and U-Boot first and write no manifest, so they cannot build an image with tagged packages.
3. `omarchy-apply-hardware` starts with `install/hardware/platform-guard.sh`. On Apple Silicon and Qualcomm it fails when the hook or its script is missing; elsewhere, where no platform packages exist, it logs that and continues, so a settings package from before the guard (the dev pair is not version locked) cannot stop an x86 install. With the guard present it runs `omarchy-platform-guard --installed`, which checks every installed package against the platform, so anything an installer placed without the guard is caught. With ticket 41's deferred hardware setup, an image build queues this step with the rest and runs it on the machine's first boot; a failure there does not stop first boot (`omarchy-provision-hardware` exits 75), but the steps queued after it wait until it passes. System setup before it installs no packages. `test/shell.d/platform-guard-test.sh` checks this order.
