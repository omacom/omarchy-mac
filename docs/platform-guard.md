...
2. Platform packages come in a later transaction: the Apple list of `omarchy-pkg-defaults apple-silicon`, the Apple kernel and the boot chain included, so none of them may be in a first pacstrap. An Apple image builder must install the settings package first, write the image manifest, then install the Apple set. The Mac image builder (`image-builder/bin/build-mac-image` in omacom/omarchy-mac-installer) does: the base system with the settings package, then the manifest, the runtime with the base and aarch64 lists, and the Apple set last. It also takes the runtime and settings packages from the same commit at the same version, so an image whose runtime runs this step always carries the guard. The mx-mac era builders do not follow this order: they pacstrap the Aurora kernel, m1n1 and U-Boot first and write no manifest, so they cannot build an image with tagged packages.
