...
- Everything else stays upstream: the @factory clone, identity and account scrub, provisioning markers and units, LUKS discovery, the passphrase check (which keeps `--token-type passphrase-only`, not yet in upstream's), the throwaway slot, and the subvolume switch. The Mac's first-boot markers are `reset-prepare`'s (ticket 81).
...
- Phase `luks` adds the new password with the recovery key and retires every other slot, so the disk ends with the two keys alone. Phase `accounts` sets the login user's and root's passwords, then runs `luks-slots owner=<slot> recovery=<slot>` where it resolves. The journal (`/var/lib/omarchy/drive-recover.state`) holds the disk's UUID, the recovery slot, the login user and the phase. A boot unlocked with the recovery key can always start phase `luks` again, with a password chosen again; one unlocked with the new password finishes phase `accounts` without asking.
- Owner provisioning arms both units (`omarchy-drive-recover --arm`) once the owner acknowledged a recovery key, so only a platform that records a recovery slot gets them. A migration arms them on encrypted Macs set up before. Snapshot boots and read-only roots never arm the reset.
...
| `scrub_factory_boot_state` and `arm_reset_markers` in `omarchy-system-factory-reset` | `reset-prepare` | Ticket 81: the runtime's reset is upstream's, which arms only owner setup (`provisioning/pending`, `wipe-pending`). After the boot-file backup, the entrypoint drops the fresh image's first-boot state from the factory root (the `mac-first-boot/deferred-steps` conversion token, and `install.conf` there and under `boot/efi/omarchy`) and arms `mac-first-boot/pending`. It drops the same state and owner setup's markers from the sealed `@factory` beside the factory root, unsealing it only when one is there. A factory root with no `@factory` beside it is refused. An `omarchy-mac-boot` with this must publish before a runtime without the inline handling: otherwise a reset leaves the factory root without the Mac's first boot, which makes its package keyring, or with an older image's conversion token. |
| `omarchy-mac-boot-update` | `boot-rebuild` | Not shipped: `boot-rebuild` stays optional until a platform ships it, and until then Apple refreshes stale entries with `limine-update`, as #527 did. A thin entrypoint around the existing command would do. The update path does not call it. |
| `omarchy-apple-silicon-boot-check` | `update-verify` | `entrypoints/update-verify` (ticket 35) runs the check limited to the boot chain (`--boot-chain`), with the new kernel's reboot allowed to be pending: the kernel and initramfs in `/boot`, the root's unlock (sd-encrypt, or busybox `encrypt` and `cryptdevice=` on a GRUB Mac installed before Omarchy's images) and the keyboard layout its passphrase prompt types with, the device-tree set, m1n1 stage 2 and U-Boot on the system ESP, and Limine's loader, menu and UKI on that same ESP. It holds only the kernel image, device trees and m1n1 against their packages, checks the kernel the boot menu starts first when both kernels are installed, and leaves out LUKS keyslots, provisioning leftovers and an m1n1 image its owner took over with `M1N1_UPDATE_DISABLED`. On failure it says not to reboot and how to rebuild the boot files. |
