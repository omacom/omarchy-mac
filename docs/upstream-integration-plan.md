...
The launch kernel remains open. Aurora may be the default if it works well enough on the agreed launch hardware and is delivering the packages needed for installation and ongoing updates. Both hardware qualification and actual package delivery are required; a working source build or installer image alone is insufficient. Record the package provider, signed artifacts, compatible dependencies and update/recovery evidence before selecting it. Asahi remains the validation baseline and fallback while that decision is open. Keep kernel choice independent of the desktop channel. Selecting Aurora for fresh installations does not authorize switching existing systems; that transition requires separate evidence and an explicit rollout decision.

Marcelo's [installer 2.0.7 change](https://github.com/maralcbr/omarchy-mx-mac/pull/197), reviewed September 21, removes the separate RC Aurora option because RC now carries Aurora. This UI change does not decide our launch default or establish package delivery and compatibility with our shared system. We have not validated that path against the distilled branch or package candidate. Confirm its exact builds and behavior before adopting it into the shared delivery path.

**DJ, Eryk, Ryan Murray and other kernel/graphics contributors, with Marcelo:** please propose the launch-default and fallback stacks here, including whether Aurora meets the hardware and package-delivery conditions. Identify the compatible kernel, modules, headers, firmware/boot components and Mesa builds; named models; update and fallback behavior; and physical-hardware acceptance checks. Explain how the Apple proposal fits the wider unified-kernel initiative. Audit `linux-asahi` assumptions and distinguish running, installed and next-boot kernels.
...
The immediate work is to deliver the validated add-on with its matching runtime/settings packages and qualify Marcelo's installer against that package set while agreeing the kernel and MLX interfaces needed for one reproducible encrypted launch candidate. Select Aurora as the default only if its hardware qualification and package delivery meet the launch requirements; otherwise proceed with the qualified Asahi path. Ready upstream fixes can continue in parallel.
