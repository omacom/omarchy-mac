...
The immediate work is to deliver the validated add-on with its matching runtime/settings packages and qualify Marcelo's installer against that package set while agreeing the kernel and MLX interfaces needed for one reproducible encrypted Asahi candidate, with an explicitly scoped Aurora preview proposal. Ready upstream fixes can continue in parallel.
