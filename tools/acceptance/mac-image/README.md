...
| `recovery-reset` | The owner has only the recovery key. Unlocked with it at the disk prompt, the disk offers a new password before the login screen; the harness chooses one and the guest loses power as the disk starts to change. The next boot, unlocked with the recovery key again, resumes the reset with a password chosen again. Afterwards the new password and the recovery key are the disk's only keys, `encrypt.state` records their slots and the journal is gone, and the next boot unlocks with the new password, offers no reset, and the new password logs in the owner and root. Skipped while the image has no `/usr/bin/omarchy-drive-recover` (ticket 70). |
...
`fresh-install` (the default) is `first-boot`, `conversion` and `second-boot`, and `all` is every scenario. `recovery-reset` and the last three check the image for their entrypoint. `snapshot-restore` and `factory-reset` fail once an image ships theirs, until they are automated, so they can never pass without having run.
...
- **Console:** the guest has no display. Owner setup and the password reset with the recovery key ask on tty1, so credential drop-ins on the kernel command line move them to the serial console and order the serial login after them. The harness answers setup there as an owner would, keeping every default. The disk password prompt appears on the serial console with Plymouth disabled (`plymouth.enable=0`).
