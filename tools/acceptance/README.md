...
- [`mac-image/`](mac-image/README.md) runs a built Mac image's lifecycle in named, independently runnable scenarios: first boot, conversion and re-key, second boot and password change, plus update, snapshot restore and factory reset once the image ships their entrypoints.
...
- `test/mac-image-run-test.sh` covers the `mac-image` runner's scenario selection and refusals, with no VM.

`asahi-fresh` exports its evidence to `~/vm-evidence/<run-id>/`, checked against `SHA256SUMS`, with the log hashes an acceptance record takes in `run.txt`, then deletes a passing run's ~23 GB run directory; a failed run keeps it under `asahi-fresh/test-runs/runs/<run-id>/` until you delete it. `mac-image` writes its serial logs, the guest's first-boot and owner-setup logs, a `run.txt` of `ok`, `not ok` and `skip` lines with the image's identity, and `SHA256SUMS` straight to `~/vm-evidence/mac-image-<run-id>/`, and a failed run keeps `~/vm-mac-image/runs/<run-id>/`. Check for 10 GiB free before a run, since updates on the host refuse below that.
