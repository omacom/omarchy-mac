...
(
  ...
  echo "unreachable"
) >"$work_dir/console.out" 2>"$console_file"
status=$?
set -e
...
[[ $(cat "$console_file") == "Error: setup failed in $failing_script (exit code: 1). See $log_file for details." ]] ||
  fail "file logging identifies the failed script, status, and log path on stderr"
[[ ! -s $work_dir/console.out ]] || fail "file logging keeps the failure report off stdout"
