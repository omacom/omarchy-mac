...
grep -Fq 'Quickshell.execDetached(["bash", applyScript])' "$plugin/Service.qml" &&
  fail "the Touch Bar service does not execDetached apply.sh"
grep -Fq 'Process {' "$plugin/Service.qml" ||
  fail "the Touch Bar service runs apply.sh as a Process so failures are visible"
grep -Fq 'omarchy-notification-send' "$plugin/Service.qml" ||
  fail "the Touch Bar service notifies when apply.sh fails"
grep -Fq 'pkexec' "$apply" || fail "apply.sh uses pkexec when there is no terminal"
grep -Fq 'sudo systemctl restart tiny-dfr.service 2>/dev/null || true' "$apply" &&
  fail "apply.sh does not swallow tiny-dfr restart failures"
pass "saving an overlay prompts for privilege and reports failures"

cat >"$stub_bin/pkexec" <<'SH'
#!/bin/bash

printf 'pkexec' >>"$TEST_LOG"
printf '\t%s' "$@" >>"$TEST_LOG"
printf '\n' >>"$TEST_LOG"
exec "$@"
SH
...
elevate_env() {
  PATH="$stub_bin:$PATH" \
    TEST_LOG="$calls" \
    OMARCHY_TOUCHBAR_LAYOUT="$test_tmp/overlay.json" \
    OMARCHY_TOUCHBAR_TOML="$test_tmp/generated.toml" \
    OMARCHY_TOUCHBAR_ETC="$conf_dst" \
    "$@"
}

: >"$calls"
printf 'stale tiny-dfr config\n' >"$conf_dst"
elevate_env bash "$apply" </dev/null >/dev/null
grep -q $'^pkexec\t/bin/bash\t-c\t' "$calls" ||
  fail "a detached apply uses pkexec, not sudo" "$(cat "$calls")"
grep -q 'systemctl restart tiny-dfr.service' "$calls" ||
  fail "pkexec restarts tiny-dfr after installing the layout" "$(cat "$calls")"
! grep -q $'^sudo\t' "$calls" ||
  fail "a detached apply does not call sudo" "$(cat "$calls")"
grep -q 'Generated by omarchy.touchbar' "$conf_dst" ||
  fail "pkexec install writes the generated layout"
pass "detached apply.sh installs through pkexec"

: >"$calls"
elevate_env bash "$apply" </dev/null >/dev/null
! grep -q pkexec "$calls" ||
  fail "an already-installed matching layout does not re-prompt" "$(cat "$calls")"
pass "existing matching tiny-dfr config skips elevation"
...
SH
printf 'stale tiny-dfr config\n' >"$conf_dst"
if elevate_env bash "$apply" </dev/null >/dev/null 2>"$test_tmp/apply.err"; then
  fail "a failed tiny-dfr restart is not ignored"
fi
pass "a failed tiny-dfr restart is reported"
