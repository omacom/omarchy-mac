...
hostile: $overridden"
  ...
  pass "no unprivileged user namespace; skipping the root environment probe"
fi
...
# The Apple predicate, the mkinitcpio baseline, the services and the package
# lists of an Apple image built on an x86 host, and of a generic image built
# on a Mac.
...
hooks_of() {
  in_world "$1" "$BASH" -c '. "$1" || exit 1; echo "${HOOKS[*]}"' -- "$ROOT/etc/mkinitcpio.conf.d/00-omarchy-hooks.conf"
}
apple_hooks="base systemd plymouth autodetect microcode modconf kms keyboard sd-vconsole block filesystems fsck"
generic_hooks="base udev plymouth keyboard autodetect microcode modconf kms keymap consolefont block encrypt filesystems fsck btrfs-overlayfs"
[[ $(hooks_of x86-host) == "$apple_hooks" ]] || fail "an Apple image build gets the Apple HOOKS baseline" "$(hooks_of x86-host)"
[[ $(hooks_of on-a-mac) == "$generic_hooks" ]] || fail "a generic image built on a Mac gets the generic HOOKS baseline" "$(hooks_of on-a-mac)"
if in_world arm-generic "$BASH" -c '. "$1"' -- "$ROOT/etc/mkinitcpio.conf.d/00-omarchy-hooks.conf" 2>/dev/null; then
  fail "an invalid manifest stops the initramfs build"
fi
pass "the mkinitcpio HOOKS baseline follows the image target, and an invalid manifest stops it"
...
services_of() {
  SYSTEMCTL_CALLS="$test_tmp/systemctl-$1"
  rm -f "$SYSTEMCTL_CALLS"
  SYSTEMCTL_CALLS=$SYSTEMCTL_CALLS in_world "$1" env PATH="$stub_bin:$test_tmp/worlds/$1/bin:$ROOT/bin:$PATH" \
    bash "$ROOT/install/config/enable-services.sh" || return 1
  cat "$SYSTEMCTL_CALLS"
}
services=$(services_of x86-host) || fail "services are enabled in an Apple image build"
[[ $services != *"systemd-oomd"* && $services == *"enable sddm.service"* ]] ||
  fail "an Apple image build leaves systemd-oomd off" "$services"
services=$(services_of on-a-mac) || fail "services are enabled in a generic image build"
[[ $services == *"enable systemd-oomd.service"* ]] || fail "a generic image built on a Mac turns systemd-oomd on" "$services"
pass "enable-services follows the image target"
...
[[ $packages == "$(OMARCHY_PATH="$ROOT" "$ROOT/bin/omarchy-pkg-defaults" apple-silicon)" ]] ||
  fail "an Apple image build installs the Apple package set"
