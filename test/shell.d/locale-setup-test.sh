...
/usr/bin/grep -q '^  ensure_utf8_locale$' "$ROOT/install/helpers/mac-install.sh" ||
  fail "the installer sets a UTF-8 locale"
locale_call=$(/usr/bin/grep -n '^  ensure_utf8_locale$' "$ROOT/install/helpers/mac-install.sh" | cut -d: -f1)
packages_call=$(/usr/bin/grep -n 'exec bash /usr/share/omarchy/install/helpers/mac-install.sh' "$ROOT/install/helpers/mac-install.sh" | cut -d: -f1)
