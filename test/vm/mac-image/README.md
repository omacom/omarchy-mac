...
An explicit `--attempt-label retry-2` instead uses new `vm/state-attempt-1/retry-2`, `vm/run-retry-2` and `vm/vm-retry-2-launch.log` paths. Labels permit only a leading lowercase letter followed by lowercase letters, digits or hyphens, up to 32 characters. Each attempt performs fresh authenticated admission; existing attempt directories and logs remain exclusive and cannot be reused. Nested state stays inside the existing loop-rule scope. Omitting the label preserves the original attempt1 paths.

Generic-kernel public-key verification uses an exclusive mode-0700 `/tmp/omarchy-vm-gpg-*` home so GPG's Unix sockets fit the Linux path limit regardless of the evidence path length. Import, signature and cleanup output remain in the attempt evidence; `kernel-keyring-home.json` records socket length and owned agent/home cleanup. The owned agent is stopped and the temporary home removed on import or signature failure as well as success. The vendor keyring still comes from the authenticated image, and automatic key retrieval is disabled.

Docker's private `/dev` may omit loop partition nodes created after container startup even when their sysfs and host udev records exist. After each owned attachment, `loop_nodes.py` requires a container `/dev` tmpfs distinct from host `/dev`, checks the exact backing and expected partition inventory, repeats each host guard, and creates only missing child block nodes using sysfs major/minor values and mode 0600. A linked, non-block or mismatched existing node fails; correct existing nodes remain unchanged. Per-attachment receipts record nodes created before any partial failure. After detach and confirmed release, cleanup removes only those created nodes with matching inode, device, mode and major/minor. The host `/dev` is never bound broadly or modified.
...
## Optional disposable ZIP source

After preserving and verifying the exact ZIP on the approved M4, a caller may explicitly stage a disposable copy in a newly created `/tmp/quattro-limine-vm-payload-20260922.XXXXXX` directory. This must be tmpfs, owned by the invoking user, mode 0700, and contain only the expected ZIP basename. The ZIP must have the same owner, mode 0400, exactly one hard link, and the descriptor-bound image report's exact size/hash. Existing artifact and catalog copies are never deleted by this harness.

Pass `--disposable-payload /tmp/quattro-limine-vm-payload-20260922.XXXXXX/omarchy-2026.09.22-aarch64-apple-silicon-limine-private-os-package.zip` to `launch_private.py vm`. The launcher records directory/file inode and ownership identities, rehashes the ZIP, requires available memory for both signed-input snapshots plus 1 GiB, and mounts only the temporary directory read-write. The ordinary artifact, builder and signed-input mounts remain read-only. A directory bind is required: a bind of the ZIP file itself would retain its inode after unlink.

`admit.py` and every descriptor pin remain unchanged. Only after admission returns successfully does `run` invoke the release helper, before its existing disk guard, kernel build, loop attachment or guest. The helper checks the admission and descriptor bindings again, rejects changed identities, owners, modes, links or file mounts, takes a Linux write lease, rehashes, and unlinks only the pinned basename through its pinned directory descriptor. The [Linux lease contract](https://man7.org/linux/man-pages/man2/F_SETLEASE.2const.html) rejects a write lease while another open file description holds the inode. It aborts on a contested lease. After close, the helper requires the allocated tmpfs space to have returned and the directory to be empty; failure prevents guest execution. The parent may remove that empty staging directory after container exit.

The selected ZIP is about 3.982 GiB; signed-input staging is about 1.830 GiB. Their combined peak is about 5.812 GiB before verifier overhead. Recheck memory before making the temporary ZIP copy. Its allocation and the verifier tmpfs must both be released before starting the 6 GiB guest. Every existing disk-space guard remains enforced, and a failed admission keeps the temporary ZIP for diagnosis. These staging operations never remove the original M4 copy or local archive/catalog paths.
